-- loadstring(game:HttpGet("https://raw.githubusercontent.com/L3monnn/ROBLOX/main/RarityMiner.lua"))()
print("HI")
--// init venyx
...
local Cubes = workspace.Places.World.Cubes
--local ChestsFolder = workspace.Places.World.Chests
    ...
    if RareOreDetecter then
        ...
        bindableFunction.OnInvoke = function(buttonPressed)
            if buttonPressed == "Teleport" then
                player.Character.HumanoidRootPart.CFrame = cube.CFrame
            end
...
local function minespecificblock(cube)
    ...
    local pickaxe = player:FindFirstChild("StarterGear") and player.StarterGear:FindFirstChild("Pickaxe")
    ...
    task.spawn(function()
        for i = 1, hitsNeeded do
            ...
            game:GetService("ReplicatedStorage").REM:FindFirstChild(MineRemoteName):FireServer(unpack(args))
        end
    end)
end
...
local function initCubes()
    ...
    local function processCube(cube)
        if RareOreDetecter then
            local timeout = 0
            while cube and (not cube:GetAttribute("TrueRarity") or not cube:GetAttribute("CubeName")) and timeout < 15 do
                task.wait(0.2)
                timeout = timeout + 1
            end
            ...
            if table.find(RareOres, cubeName) then
                highlightCube(cube)
    ...
    for _, cube in pairs(Cubes:GetDescendants()) do
        if cube.ClassName == "Part" then
            if cube.Name == "Main" then
                cube = cube.Parent
            end

            if cube.Parent and not cube.Parent.ClassName == "Model" then
                continue
            end

            if cube:GetAttribute("CubeName") == nil then
                continue
            end
    ...
    CubeAddedConn = Cubes.DescendantAdded:Connect(function(cube)
        if cube.ClassName == "Part" then
            if cube.Parent and not cube.Parent:IsA("Model") then
                continue
            end
            ...
            if AutoMineBlocks then
                ...
                if distance <= MineAuraRadius then
                    task.spawn(minespecificblock, cube)
                    dontDetect = true
                end
            end
    ...
    CubeRemovedConn = Cubes.DescendantRemoving:Connect(function(cube)
        if cube.ClassName == "Part" then
            Main[cube.Name] = nil
        end
    end)
...
local function MineCubesNearPlayer()
    ...
    for cubeName, cube in pairs(Main) do
        task.spawn(function()
            if cube then
                local distance = (cube.Position - playerPosition).Magnitude
...
local function SellMaterials()
    ...
    if SellRemoteName then
        ...
        local JimmyPenny = workspace.Places.World.Lobby.NPCs:FindFirstChild("Jimmy Penny")
        ...
        char.HumanoidRootPart.CFrame = JimmyPos
        task.wait(0.3)
    ...
    if RollRemoteName then
        ...
        for i = 1, TraitTimeout do
            ...
            if result == TraitRarity then
                notifyUser("Success!", "Successfully rolled the " .. TraitRarity .. " Trait", 3, successDecalID)
                break
    ...
    if not checkcaller() and self.ClassName == "RemoteFunction" and method == "invokeserver" then
        if args[1] and typeof(args[1]) == "table" then
            for i, v in pairs(args[1]) do
                if i == "ToSell" and typeof(v) == "table" and not SellRemoteName then
    ...
    elseif not checkcaller() and self.ClassName == "RemoteEvent" and method == "fireserver" then
        if args[1] and typeof(args[1]) == "table" then
            for i, v in pairs(args[1]) do
        		...
        		elseif i == "Position" and typeof(v) == "string" and not MineRemoteName then
